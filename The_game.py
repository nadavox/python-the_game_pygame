...
class SPACE_SHIP:
    ...
    def update_rect(self):
        self.rect = self.spaceship.get_rect(midleft = (self.rect_spaceship) + (0,17))
...
class BULLET(pygame.sprite.Sprite):
    def __init__(self, width, hegiht, pos_x, pos_y, color, diriction):
        ...
        if self.diriction == 'right':
            self.rect.midleft = [pos_x + 60, pos_y + 8]
        if self.diriction == 'left':
            self.rect.center = [pos_x - 30, pos_y + 8]
...
class COIN:
    def __init__(self, collect  = False, SuperPower = False,):
        ...
        if self.SuperPower:
            ...
            self.rect = self.coin.get_rect(center=(self.position))
...
def coin_collect(coin, spaceship):
    coin.draw()
    # pygame.draw.rect(screen, (0,255,255), coin.rect)
    # pygame.draw.rect(screen, (255,0,0), spaceship.rect)
    if pygame.sprite.collide_rect(coin, spaceship):
        print("work5")
        coin.collect_or_not()
        return 1

    elif coin.rect.topleft[0]  <= spaceship.rect.midleft[0] <= coin.rect.topright[0] and\
        coin.rect.topright[1] <= spaceship.rect.midleft[1] <= coin.rect.bottomright[1]:
        coin.collect_or_not()
        print('work3')
        return 1
