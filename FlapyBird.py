...
def draw_pipes(pipes):
    for pipe in pipes:
        if pipe.bottom >=1024:
            ...
            flip_pipe = pygame.transform.flip(pipe_surface, False, True) # flip the surface when (x direction, y direction)
...
bg_surface = pygame.image.load('assets/background-day.png').convert() #varible for background_surface, convert is to help the game to run more easliy
...
while True: # the game loop
    for event in pygame.event.get(): # every thing i do in the window of the game
        ...
        if event.type == SPAWNPIPE:
            pipe_list.extend(create_pipe()) # make a new pipe and insert to a list of pipe bt a function call create pipe)
